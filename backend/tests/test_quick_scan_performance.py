"""Performance test for Stockfish analysis - sequential vs parallel with engine pool."""
...
def run_performance_test(username: str = "elizura", num_games: int = 10, mode: str = "both") -> None:
    """Run analysis performance test.
    
    Args:
        username: Lichess username
        num_games: Number of games to analyze
        mode: "sequential", "parallel", or "both"
    """
    ...
    if mode == "both":
        speedup = seq_stats["total_time"] / par_stats["total_time"] if par_stats["total_time"] > 0 else 0
        print(f"\n{'='*70}")
        print("COMPARISON")
        print("="*70)
...
if __name__ == "__main__":
    ...
    mode = sys.argv[3] if len(sys.argv) > 3 else "both"  # sequential, parallel, or both
    ...
    run_performance_test(username, 250, "parallel")
